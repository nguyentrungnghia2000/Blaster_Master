...
{
	UIanimationSet->at(PLAYER_HEALTH_BAR)->Render(x + 3, y + ARTICULAR_POWER_HEALTHBAR_Y);
	UIanimationSet->at(PLAYER_HEALTH_BAR)->Render(x + 3, y + ARTICULAR_PLAYER_HEALTHBAR_Y);
	//PowerHB->Render();
	PlayerHB->Render();
	//DebugOut(L"carHUD : %d\n", CAR_HEALTH_UNIT);
}
